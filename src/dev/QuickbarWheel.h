...
//  is compiled OUT of distribution builds (EMOT3_DIST) - exactly like the
//  dev overlays (Profiling.h / QuickbarDebug.h). In dist these are no-op
//  stubs and the Quickbar falls back to its original grace-period
...
// keep the game from also scrolling/zooming). Always false in dist.
...
// io.MouseWheel; positive = up). Returns 0 in dist / when nothing was captured.
float QbWheelDrain();
