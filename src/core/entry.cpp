...
"bundled image is used again on next load.\n";
        ...
        // First-run README next to me_motes.json — explains what the
        // /me-mote catalog is, the three-variant schema, the icon
        // resolution chain, and how to hand-edit the JSON for users who
        // prefer a text editor to the Options > /me-motes tab. Idempotent
        // (only writes when missing), so a user-edited or deleted README
        // survives subsequent launches.
        {
            std::string mmReadmePath = std::string(addonDir) + "\\me_motes_README.txt";
            DWORD attr = GetFileAttributesA(mmReadmePath.c_str());
            if (attr == INVALID_FILE_ATTRIBUTES) {
                std::ofstream rf(mmReadmePath);
                if (rf.is_open()) {
                    rf <<
"emot3 - /me-motes\n"
"=================\n"
"\n"
"/me-motes are short text snippets you fire as GW2 /me chat\n"
"commands - a \"press one button to broadcast something specific\"\n"
"shortcut that the official emote catalog doesn't cover (and that no\n"
"animation matches). The catalog lives in me_motes.json next to this\n"
"file; the friendly way to edit it is the Options > /me-motes tab\n"
"in-game.\n"
"\n"
"Three variants per /me-mote\n"
"---------------------------\n"
"Each /me-mote ships up to three pre-written bodies. The cell click\n"
"and right-click menu pick between them at send time:\n"
"\n"
"  Default text   Left-click sends this body. REQUIRED.\n"
"  You-variant    Right-click -> \"Send (you)\" sends this body. Use\n"
"                 for \"asks you to ...\", \"tells you ...\". Optional;\n"
"                 the menu entry grays out when empty.\n"
"  All-variant    Right-click -> \"Send (all)\" sends this body. Use\n"
"                 for \"tells the group...\", \"asks the group ...\".\n"
"                 Optional; same.\n"
"\n"
"GW2 has no addon-facing target name, so target-aware auto-switching\n"
"isn't possible - the variants are the workaround. Pick the context\n"
"at send time instead of letting the addon guess.\n"
"\n"
"What gets sent\n"
"--------------\n"
"emot3 prepends \"/me \" to whichever variant you pick. DON'T type\n"
"\"/me\" into the body field; the prefix is added at send time (an\n"
"accidental one gets stripped on save). GW2 caps each chat line at\n"
"~199 characters; with the prefix taking 4, that leaves 195 for the\n"
"body. The Options editor shows a live char count under each input\n"
"and warns when you cross the cap.\n"
"\n"
"Icon resolution (in priority order)\n"
"-----------------------------------\n"
"  1. Explicit IconPath - set via the Browse button in Options >\n"
"     /me-motes (or hand-typed into the JSON's \"icon\" field).\n"
"  2. icons/<id>.png    - drop a PNG into addons/emot3/icons/ named\n"
"                         after the /me-mote's stable id, and it\n"
"                         auto-binds with no Browse roundtrip.\n"
"  3. Bundled AI art    - opt-in via Options > General -> \"Use\n"
"                         AI-generated fallback icons\". emot3 ships\n"
"                         a small set of thematic icons keyed on\n"
"                         common ids (lfg, brb, idea, clap, ...).\n"
"                         A /me-mote whose id matches one auto-gets\n"
"                         the bundled icon when the toggle is on.\n"
"  4. Letter fallback   - styled button with the /me-mote's first\n"
"                         letter. Always available; no setup.\n"
"\n"
"Bundled samples\n"
"---------------\n"
"emot3 ships a handful of sample /me-motes in the social-coordination\n"
"lane (LFG, Need help, BRB, To waypoint, Ready to pull, Mount up).\n"
"They seed on the first run in the emote language you pick. The\n"
"\"Restore bundled\" button in Options > /me-motes re-adds any sample\n"
"you've deleted; it's idempotent so it never duplicates a /me-mote\n"
"you already have, and it never overwrites one you've edited.\n"
"\n"
"Hand-editing me_motes.json\n"
"--------------------------\n"
"Schema (version 1):\n"
"\n"
"  {\n"
...
"    \"me_motes\": [\n"
"      {\n"
"        \"id\":       \"lfg\",       (stable, [a-z0-9_])\n"
"        \"name\":     \"LFG\",       (display label)\n"
"        \"icon\":     \"\",          (empty -> icon chain above)\n"
"        \"aliases\": [\"group\", \"party\"], (Library search words)\n"
"        \"text\":     \"is looking for a group.\",      (Default)\n"
"        \"text_you\": \"asks you to join the group.\",  (optional)\n"
"        \"text_all\": \"calls out for a group.\"        (optional)\n"
"      }\n"
"    ]\n"
"  }\n"
"\n"
"emot3 sanitizes on load (trims whitespace, strips leading \"/me \",\n"
"normalizes ids to [a-z0-9_], dedupes by id, drops entries with no\n"
"id or no default text). Anything it heals is logged via the Nexus\n"
"log window and re-saved to disk on the spot.\n";
                    LOG_DEBUG("Wrote default /me-motes README to %s",
                              mmReadmePath.c_str());
                } else {
                    LOG_WARNING("Could not write %s - users won't see /me-mote catalog docs",
                                mmReadmePath.c_str());
                }
            }
        }
